...
def std_atmosphere(ha):
    ...
    P1 = [2116.21662, 472.688, 114.345, 18.129, 2.31634, 1.39805, 0.010082632, 0.0077986]
...
while loop:
    print('')
    # user input
    h_in = input('altitude (ft): '.ljust(16))
    
    h_in = float(h_in)
    if not (int(h_in) in range(0, 390000)):
        raise ValueError('altitude must be between 0 and 390k ft')
    
    # define the standard atmosphere
    delta, theta, sigma, Ph, Th, Rhoh, ah, muh, Ph_SI, Th_SI, Rhoh_SI, ah_SI, muh_SI = std_atmosphere(h_in)
